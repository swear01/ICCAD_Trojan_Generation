...
class SimpleTrojanGenerator:
    ...
    def read_host_circuit(self, trojan_id: str) -> str:
        """Read the host circuit file"""
        host_file = self.config_loader.get_host_file(trojan_id)
    ...
    def generate_verilog_module(self, trojan_id: str, params: Dict[str, Any], 
        ...
        host_circuit = self.read_host_circuit(trojan_id)
        trojan_core = self.read_trojan_core(trojan_id, variant)
        
        # Inject parameters into both host and trojan core
        host_circuit = self.inject_parameters(host_circuit, params)
        trojan_core = self.inject_parameters(trojan_core, params)
...
"""
    ...
    def generate_circuit_pair(self, trojan_id: str, instance_id: int) -> tuple:
    ...
    def generate_batch(self, num_circuits: int = 10, trojans: List[str] = None) -> None:
        ...
        if trojans is None:
            trojans = self.config_loader.get_all_trojan_ids()
        
        summary_data = []
        
        for trojan_id in trojans:
            # Create subdirectories for each trojan type
            ...
            print(f"Generating {num_circuits} instances for {trojan_id}...")
            ...
            for i in range(num_circuits):
                ...
                summary_data.append({
                    ...
                    'parameters': params,
                    'description': self.config_loader.get_description(trojan_id)
                })
...
def main():
    ...
    generator = SimpleTrojanGenerator(args.output_dir)
